...
def parse_args():
    ...
    parser.add_argument(
        "--device_map",
        default=None,
        help="Hugging Face device map (e.g. 'auto') for sharding a model across GPUs. "
    ...
    parser.add_argument(
        "--rdfw",
        action="store_true",
        help="Whether to use RDFW",
    )
    parser.add_argument(
        "--srdfw",
        action="store_true",
        help="Whether to use SRDFW",
    )
    parser.add_argument(
        "--gamma",
        type=float,
...
def main():
    ...
    if args.device_map and accelerator.state.num_processes > 1:
        raise ValueError(
            "device_map is not compatible with multi-process launch. "
            "Run with a single process (e.g. set --num_processes=1) to enable sharding."
        )
            ...
            if args.sweet:
                if args.model != "meta-llama/Llama-2":
                    model_kwargs = {
                        ...
                        "low_cpu_mem_usage": bool(args.device_map),
                    }
                    if "use_auth_token" in inspect.signature(
